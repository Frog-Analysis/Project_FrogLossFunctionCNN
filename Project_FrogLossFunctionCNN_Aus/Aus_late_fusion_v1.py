...
method_list = method_list[7:]
for method in method_list:
    print(method)
    
    name_list = os.listdir(os.path.join(baseFolder, method))
    for name in name_list:
        print(name)
    
        method_folder = os.path.join(baseFolder, method, name,
...
class_acc_fusion1 = temp_cm.diagonal()/temp_cm.sum(axis=1)
# print(class_acc_fusion1)

save_path = './plot_new' + '/best_cm.csv'
np.savetxt(save_path, temp_cm, delimiter=",", fmt='%s')

accuracy_result = accuracy_score(testLabel, predict_label_test)
fscore_result = f1_score(testLabel, predict_label_test, average='macro')    
...
print([accuracy_result, fscore_result, kapppa_score])
