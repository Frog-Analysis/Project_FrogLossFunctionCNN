% Segment continuous recordings into blocks with fixed durations
...
for i = 1:length(train_percent_array)
    ...
    training_len_array = zeros(1, nSpecies);
    for iSpecies = 1:nSpecies
        ...
        training_len_array(iSpecies) = training_len;
        
        % training and testing data 
        training_folder =  ['.\Australia-Frog\data_split\training_', num2str(train_percent), '\'];
        testing_folder = ['.\Australia-Frog\data_split\testing_', num2str(train_percent), '\'];
    ...
    csvwrite([save_folder_len, 'training_len_', num2str(train_percent), '.csv'], training_len_array);
